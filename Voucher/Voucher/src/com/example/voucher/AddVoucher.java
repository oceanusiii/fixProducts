	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		code_voucher.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View arg0, boolean arg1) {
				...
				Log.d("list",
						arrayCodeVoucher.toString() + arrayCodeVoucher.size());
				for (int i = 0; i < arrayCodeVoucher.size() - 1; i++) {
					if (arrayCodeVoucher.get(i).equals(a)) {
						code_voucher
								.setError(" Code da bi trung hoac ban chua nhap ");
		...
		save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (checkCodeVoucher) {
					...
					Toast.makeText(AddVoucher.this,
							" Moi nhap lai code voucher ", Toast.LENGTH_SHORT)
							.show();
			}
		});
	}
		...
		protected String doInBackground(String... arg0) {
			...
			try {
				try {
					...
					arrItem = mWS.connectWSPut_Get_Data(urlgetall, json,
							"voucher");
					if (arrItem != null) {
		...
		protected String doInBackground(String... arg0) {
			...
			try {
				try {
					JSONObject json = new JSONObject();
					JSONArray arrItem = new JSONArray();
					arrItem = mWS.connectWSPut_Get_Data(urlidmax, json,
							"voucher");
					if (arrItem != null) {
